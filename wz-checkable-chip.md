# `wz-checkable-chip` Component Specification

This document provides the specification for the Waze Styleguide `wz-checkable-chip` component, designed to allow AI systems to perfectly clone the visual and structural characteristics of the component.

## 1. Overview & API
**Tag:** `wz-checkable-chip`
**Encapsulation:** `shadow`

### Properties
| Property | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `checked` | `boolean` | `false` | Whether the chip is checked. When true, a check icon will appear. |
| `controlled` | `boolean` | `false` | Indicates whether the checked state of the chip is controlled from outside the component. If true, changes to the `checked` property from within the component will not toggle the chip's checked state. Instead, the parent component is responsible for updating the `checked` property. If false, the chip's checked state will toggle internally when clicked. |
| `disabled` | `boolean` | `false` | Whether the chip is disabled. |
| `error` | `boolean` | `false` | Whether the chip in error state. |
| `highlighted` | `boolean` | `false` | Whether the chip is currently highlighted. |
| `isDragging` | `boolean` | `false` | Whether the chip is being dragged. |
| `readOnly` | `boolean` | `false` | Whether the chip is read-only. |
| `removable` | `boolean` | `false` | Whether the chip can be removed. When true, the chip will display an X icon for removal. |
| `showCheckIconWhenChecked` | `boolean` | `true` | Whether to show a check icon when the chip is checked. |
| `size` | `"lg" \| "md" \| "sm"` | `"md"` | The size of the chip. (default: md) Can be one of the following: 'sm' | 'md' | 'lg' |
| `value` | `number \| string` | `` | The value of the component. |

### Slots
*No slots defined.*

### Events
- `chipChanged`: Emits when interaction occurs
- `chipClicked`: Emits when interaction occurs
- `removeClicked`: Emits when interaction occurs
...
render() {
        const componentClass = {
            ...
            immutable: !this.isMutable(),
            outline: true,
            removable: this.removable,
        };
        const shouldRenderIcons = isSizeSupportsIcons(this.size);
        const shouldRenderIconSlot = shouldRenderIcons && this.hasSlot("icon");
        const shouldRenderCheckIcon = shouldRenderIcons && this.checked && this.showCheckIconWhenChecked;
        const shouldRenderRemoveIcon = shouldRenderIcons && this.removable;
        return (h("div", { key: '5adaec3fbf2e91d5ca85f3ca24edd297b52a197c', tabIndex: 0, class: componentClass }, shouldRenderIconSlot ? (h("slot", { name: "icon" })) : shouldRenderCheckIcon ? (h(CheckIcon, { size: this.size })) : null, h("span", { key: '020ef516f3f5ff457165567a8307d6409f74eef9', class: "text" }, h("slot", { key: 'c61aff171ba62ae3a5a69e9891b4ba3afee92d8e' })), shouldRenderRemoveIcon && (h(RemoveIcon, { key: '3969e4b271cde177a9b5034ef90057579f2dec49', onClick: this.onChipRemove.bind(this) }))));
    }
```
...
wz-checkable-chip .wz-chip .text {
  user-select: none;
}
```

# `wz-option` Component Specification

This document provides the specification for the Waze Styleguide `wz-option` component, designed to allow AI systems to perfectly clone the visual and structural characteristics of the component.
...
| :--- | :--- | :--- | :--- |
| `disabled` | `boolean` | `false` | Whether the current option is disabled. |
| `highlighted` | `boolean` | `false` | Whether the current option should be highlighted, e.g as a selected option or pre selected option (keyboard navigating) in wz-select |
| `value` | `number \| string` | `` | The option's value, to be read by wz-select component. |

### Slots
*No slots defined.*

### Events
- `optionClicked`: The option clicked event is being fired on option click to notify
the parent wz-select component that an option was selected, and
its value.
...
render() {
        ...
        return (h("div", { key: '4c476d67043c5d7cc4d5d6a7a25e67e3e65129c1', class: componentClass }, h("slot", { key: 'dd4815f5ba01c5580c239743ef5543ddcda7e75a' })));
    }
```
...
/**
 * @prop --wz-menu-option-height
 * @prop --wz-menu-option-height
 * @prop --wz-menu-option-padding
 * @prop --wz-menu-options-wrapper-padding
 * @prop --wz-menu-option-divider-height
 * @prop --wz-menu-option-hover-color
 * @prop --wz-menu-options-wrapper-padding
 */
::slotted(i.w-icon) {
  color: #202124;

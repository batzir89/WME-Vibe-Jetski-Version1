# `wz-multi-select` Component Specification

This document provides the specification for the Waze Styleguide `wz-multi-select` component, designed to allow AI systems to perfectly clone the visual and structural characteristics of the component.

## 1. Overview & API
**Tag:** `wz-multi-select`
**Encapsulation:** `shadow`
**Dependencies:** `wz-checkable-chip`, `wz-button`, `wz-autocomplete`

### Properties
| Property | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `disabled` | `boolean` | `` | Whether or not to disable the component, preventing user input. |
| `displayMaxlength` | `boolean` | `` | Whether or not to display the value length / maxlength counter. |
| `enableNewItems` | `"AllowNewItems" \| "NoNewItems" \| "OnlyEmptyItem"` | `` | Whether or not to allow setting the value of the search to be a string that is not in the getItems. Useful in case you want to redirect to multi-option list once the search is complete. |
| `errorMessage` | `string` | `` | The error message for error state. |
| `getItems` | `(query: string) => IAutocompleteItem[] \| Promise<IAutocompleteItem[]>` | `` | A query function that has to be provided, in order for the autocomplete to show suggestions. |
| `helperMessage` | `string` | `""` | The assistive text for the text input |
| `isCaseSensitive` | `boolean` | `` | Whether to respect case when matching query and options text on element blur. |
| `label` | `string` | `` | The label of the autocomplete input. |
| `maxlength` | `number` | `` | The max length that the AutoComplete input allows. |
| `mode` | `"Contains" \| "Fuzzy" \| "PreMatched" \| "Prefix"` | `` | The mode used for matching/sorting suggestions. |
| `noSuggestionText` | `string` | `` | The text that will be shown if there are no suggestions available. |
| `placeholder` | `string` | `` | The placeholder text in the field while there is nothing written. |
| `shouldDisplayDropdownSymbol` | `boolean` | `` | Whether or not to display the dropdown symbol. |
| `shouldMatchOnSecondaryText` | `boolean` | `` | Whether to match the query on the secondary text as well. Doesn't affect prefix mode. |
| `textChangedError` | `(reason: string) => void` | `` | The function to be called when error occurs on filtering function. By default, exposes the error reason. |
| `values` | `WzMultiSelectItem[]` | `[]` | Array of the selected Items. valuesChanged event won't be triggered when changing this prop. Check `IAutocompleteItem` type on [wz-autocomplete component page](/?path=/docs/🧩-components-autocomplete--docs). |

### Slots
*No slots defined.*

### Events
- `inputChanged`: The event which will be triggered when the user changes the AutoComplete internal input.
The event detail is the updated input value (the user query).
- `menuToggled`: The event which will be triggered when suggestion menu opens/closes.
- `valueCheckChanged`: The event which will be triggered when a value is checked/unchecked.
- `valuesChanged`: The event which will be triggered once an item is selected/removed by the user.
This event isn't triggered when changing the values prop.
The event detail is an object with the following type:
`{ values: IAutocompleteItem[]; added?: IAutocompleteItem; removed?: IAutocompleteItem[]; }`
...
```javascript
render() {
        return (h("div", { key: 'bb1785215d7f85faa5a081cb1493bb03e07fba66', id: "multi-select-container" }, h("wz-autocomplete", { key: '3155d538d7c5be062accf8c9cf959d07a9f93b65', ref: (el) => (this.autocompleteElement = el), mode: this.mode, shouldDisplayDropdownSymbol: this.shouldDisplayDropdownSymbol, shouldCloseMenuOnSelection: false, getItems: this.getItemsHandler, placeholder: this.placeholder, shouldMatchOnSecondaryText: this.shouldMatchOnSecondaryText, noSuggestionText: this.noSuggestionText, textChangedError: this.textChangedError, enableNewItems: this.enableNewItems, isCaseSensitive: this.isCaseSensitive, label: this.label, disabled: this.disabled, value: this.getAutocompleteEmptyValue(), maxlength: this.maxlength, displayMaxlength: this.displayMaxlength, errorMessage: this.errorMessage, helperMessage: this.helperMessage, shouldRenderChips: true, onItemSelected: this.itemSelectedHandler, onInputChanged: this.inputChangedHandler, onMenuToggled: this.menuToggledHandler }, this.values.length > 0 && (h("div", { key: 'aed7be68d5aaca1c3b4a62729e2f20af656ec784', class: "chip-container", slot: CHIPS_CONTAINER_SLOT }, this.renderSelectedChips())), this.values.length > 0 && (h("slot", { key: '68bd94cb26fb6c0e4254a5a3c7c28b1650f72e58', slot: REMOVE_ALL_CHIPS_SLOT }, this.renderRemoveAllButton())))));
    }
```
...
#multi-select-container .chip-container {
  display: flex;
  flex-wrap: wrap;
...
#multi-select-container .multi-select-chips-remove-all {
  margin-left: var(--space-always-xs, 8px);
  margin-right: var(--space-always-s, 12px);

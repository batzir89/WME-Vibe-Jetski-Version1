# `wz-empty-state` Component Specification

This document provides the specification for the Waze Styleguide `wz-empty-state` component, designed to allow AI systems to perfectly clone the visual and structural characteristics of the component.

## 1. Overview & API
**Tag:** `wz-empty-state`
**Encapsulation:** `shadow`
**Dependencies:** `wz-h6`, `wz-h2`, `wz-body2`, `wz-body1`, `wz-caption`, `wz-basic-tooltip`, `wz-tooltip-source`, `wz-tooltip-target`, `wz-tooltip-content`
...
```javascript
render() {
        const hasButton = this.hasSlot("button");
        const hasHeadline = Boolean(this.headline);
        const hasCaption = Boolean(this.caption);
        const image = this.getImage();
        const headline = this.getHeadline();
        const description = this.getDescription();
        const button = this.getButton();
        const captionWithIcon = this.getCaptionWithIcon();
        if (!WZ_EMPTY_STATE_SIZE.includes(this.size)) {
            console.warn("[UX] wz-empty-state component doesn't support this size");
        }
        if (!hasHeadline && this.description) {
            console.warn("[UX] wz-empty-state component should have headline if it has description");
        }
        if (!hasButton && this.caption) {
            console.warn("[UX] wz-empty-state component should have button if it has caption");
        }
        if (!hasCaption && this.captionIcon) {
            console.warn("[UX] wz-empty-state component should have caption if it has tooltip");
        }
        return (h("div", { key: '5ba042bcfed848b828cfad1657ffcdd6d3ac92d3', class: "wz-empty-state" }, image, headline, hasHeadline ? description : null, button, hasButton && hasCaption ? captionWithIcon : null));
    }
```
...
/**
  * @prop --wz-empty-state-image-width
  * @prop --wz-empty-state-image-height
  */
.wz-empty-state {
  box-sizing: border-box;
  max-width: 360px;
  padding: 0 var(--space-always-xl, 32px);
  text-align: center;
  width: 100%;
}

img {
  height: var(--wz-empty-state-image-height, auto);
  width: var(--wz-empty-state-image-width, auto);
}
